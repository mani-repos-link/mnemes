...
This repository is a lightweight, open‑source, self‑hosted chatbot for running on a personal machine using free LLMs. It uses a Vue 3 + Vite frontend (TypeScript) and Python with SQLite (including sqlite-vec for vector storage) as the sole data store.
